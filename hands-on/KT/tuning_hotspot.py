...
def tune():
    ...
    nvmlobserver = NVMLObserver(["nvml_energy", "core_freq", "mem_freq", "temperature"],
                                save_all=True, nvidia_smi_fallback="/cm/shared/package/utils/bin/run-nvidia-smi")
    ...
    metrics["GFLOPS/W"] = lambda p: ((p["temporal_tiling_factor"] * 15 * problem_size[0] * problem_size[1])/1e9) / p["nvml_energy"]
